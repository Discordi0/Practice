+ [Dreaming](../THM/Dreaming.md) 

After a file was discovered that could grant lateral movement to another user, a reverse shell was inserted into the table that was being selected from that vuln file.

+ [Gallery](../THM/Gallery.md) 

Found a login form that was susceptible to a SQLi bypass.

+ [Light](../THM/Light.md) 

Modified payloads to bypass filters, where used to first extract model and version of dbms and then credentials.

+ [Plotted-TMS](../THM/Plotted-TMS.md) 

Login form vulnerable to SQLi bypass.

+ [Poster](../THM/Poster.md) 

SQLi adjacent, an Metasploit module was used to enter payloads and extract info.

+ [Source](../THM/Source.md) 

Login form suceptible to payload bypass, but no valid payload was utilized. (went different route)